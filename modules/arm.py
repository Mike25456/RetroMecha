...
FIX: polySphere usa sh= (subdivisions height), no se=
"""
...
class ArmModule(BaseModule):
    ...
    def generate(self,
        ...
        # ── Articulación codo — sa=sudivisiones axiales, sh=subdivisiones altura
        elbow = mc.polySphere(r=0.20, sa=6, sh=4,
        ...
        parts = [upper, detail, elbow, forearm]

        # ── Spike (alta agresividad) ──────────────────────────────────────────
        if aggr > 0.4:
            spike_h = 0.25 + aggr * 0.2
            spike   = mc.polyCone(r=0.08, h=spike_h, sa=4,
                                  name='rm_arm_spike_#')[0]
            mc.move(0,
                    -(upper_h * 0.5 + 0.30 + fore_h * 0.5 + spike_h * 0.3),
